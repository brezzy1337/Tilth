...
describeWithDb("account settings (F-051) — Postgres integration", () => {
  ...
  async function seedStoreWithLocation(userId: string, name: string, lat: number, lng: number): Promise<string> {
    const [s] = await db.insert(schema.stores).values({ userId, name }).returning({ id: schema.stores.id });
  ...
  async function seedApprovedPlace(name: string, sourceRef: string, linkedUserId: string): Promise<string> {
    const [p] = await db
  ...
  async function deactivate(userId: string): Promise<void> {
    await db
      .update(schema.users)
      .set({ deactivatedAt: new Date(), deleteAfter: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) })
  ...
  afterAll(async () => {
    if (seededConversationIds.length > 0) {
      await db.delete(schema.messages).where(inArray(schema.messages.conversationId, seededConversationIds));
    }
    if (seededStoreIds.length > 0) {
      await db.delete(schema.sourcingRequests).where(inArray(schema.sourcingRequests.storeId, seededStoreIds));
    }
    if (seededConversationIds.length > 0) {
      await db.delete(schema.conversations).where(inArray(schema.conversations.id, seededConversationIds));
    }
    if (seededStoreIds.length > 0) {
      await db.delete(schema.gardenPosts).where(inArray(schema.gardenPosts.storeId, seededStoreIds));
    ...
    if (seededUserIds.length > 0) {
      ...
      await db.delete(schema.userBlocks).where(inArray(schema.userBlocks.blockerUserId, seededUserIds));
      await db.delete(schema.userBlocks).where(inArray(schema.userBlocks.blockedUserId, seededUserIds));
    }
    if (seededPlaceIds.length > 0) {
      await db.delete(schema.communityPlaces).where(inArray(schema.communityPlaces.id, seededPlaceIds));
    }
    if (seededStoreIds.length > 0) {
  ...
  describe("auth.changePassword", () => {
    ...
    it("wrong current password -> UNAUTHORIZED", async () => {
      ...
      const caller = createCaller(ctxFor(reg.user.id));
      await expect(
        caller.auth.changePassword({ currentPassword: "TotallyWrong1!", newPassword: "NewPass456!" }),
      ).rejects.toThrow(expect.objectContaining({ code: "UNAUTHORIZED" }));
    });
  ...
  describe("auth.deleteAccount", () => {
    it("happy path: sets deactivatedAt/deleteAfter (~+30d), deletes push tokens, withdraws the caller's own pending sourcing request", async () => {
      ...
      await caller.chat.registerPushToken({ token: "ExponentPushToken[da-happy]", platform: "ios" });
      ...
      const growerStoreId = await seedStoreWithLocation(growerOwnerId, "DA Happy Grower", ORIGIN_LAT, ORIGIN_LNG);
      ...
      const tokens = await db.select().from(schema.pushTokens).where(eq(schema.pushTokens.userId, reg.user.id));
    ...
    it("refuses (BAD_REQUEST) when the caller has a non-terminal order as BUYER", async () => {
      ...
      const storeId = await seedStoreWithLocation(sellerId, "DA Buyer-Open Store", ORIGIN_LAT, ORIGIN_LNG);
    ...
    it("refuses (BAD_REQUEST) when the caller has a non-terminal order as the owning STORE", async () => {
      ...
      seededUserIds.push(reg.user.id);
      const storeId = await seedStoreWithLocation(reg.user.id, "DA Seller-Open Store", ORIGIN_LAT, ORIGIN_LNG);
  ...
  describe("auth.login — deactivated account", () => {
    it("self-restores a deactivated account when logging in within the grace window", async () => {
      ...
      const anon = createCaller(ctxFor(null));
      const loginResult = await anon.auth.login({ usernameOrEmail: "restoreuser", password: "RestoreMe123!" });
    ...
    it("rejects with a generic UNAUTHORIZED once past the grace window (no state leak)", async () => {
      ...
      const wrongPasswordErr = await createCaller(ctxFor(null))
        ...
        .catch((e: unknown) => e);
      const pastGraceErr = await anon
        .auth.login({ usernameOrEmail: "pastgrace", password: "PastGrace123!" })
        .catch((e: unknown) => e);
  ...
  describe("deactivation hides selling surfaces and blocks new writes", () => {
    ...
    beforeAll(async () => {
      ...
      targetStoreId = await seedStoreWithLocation(targetUserId, "Hide Target Farm", ORIGIN_LAT, ORIGIN_LNG);
    ...
    it("listings.nearby excludes a deactivated seller's listings", async () => {
      ...
      const before = await caller.listings.nearby({ lat: ORIGIN_LAT, lng: ORIGIN_LNG, radiusKm: 10 });
      expect(before.map((r) => r.storeId)).toContain(targetStoreId);

      await deactivate(targetUserId);
      const after = await caller.listings.nearby({ lat: ORIGIN_LAT, lng: ORIGIN_LNG, radiusKm: 10 });
    ...
    it("stores.get returns NOT_FOUND for a deactivated seller's public profile", async () => {
      const caller = createCaller(ctxFor(null));
      await expect(caller.stores.get({ storeId: targetStoreId })).resolves.toMatchObject({ id: targetStoreId });
    ...
    it("sourcing.growers excludes a deactivated grower", async () => {
      ...
      const before = await placeCaller.sourcing.growers({ lat: ORIGIN_LAT, lng: ORIGIN_LNG, radiusKm: 10 });
      expect(before.map((r) => r.storeId)).toContain(targetStoreId);

      await deactivate(targetUserId);
      const after = await placeCaller.sourcing.growers({ lat: ORIGIN_LAT, lng: ORIGIN_LNG, radiusKm: 10 });
      expect(after.map((r) => r.storeId)).not.toContain(targetStoreId);
      await restore(targetUserId);
    });
    ...
    it("sourcing.createRequest: NOT_FOUND when the target grower is deactivated", async () => {
      ...
      const placeCaller = createCaller(ctxFor(placeBuyerUserId));
      await expect(
        placeCaller.sourcing.createRequest({ storeId: targetStoreId, produce: "Tomatoes", quantity: "10 lb" }),
  ...
  describe("chat.listBlocked / chat.unblockUser", () => {
    it("round-trips: block -> listBlocked shows it -> unblock -> listBlocked no longer shows it; unblock is idempotent", async () => {
      ...
      await expect(caller.chat.unblockUser({ userId: blockedId })).resolves.toEqual({ success: true });
    });
  });
  ...
  describe("chat.unregisterPushToken", () => {
    it("deletes only the caller's own token; someone else's token (or a never-registered one) is a silent no-op", async () => {
      ...
      await expect(otherCaller.chat.unregisterPushToken({ token })).resolves.toEqual({ success: true });
      const stillThere = await db.select().from(schema.pushTokens).where(eq(schema.pushTokens.token, token));
      expect(stillThere).toHaveLength(1);

      await expect(ownerCaller.chat.unregisterPushToken({ token })).resolves.toEqual({ success: true });
      const goneNow = await db.select().from(schema.pushTokens).where(eq(schema.pushTokens.token, token));
